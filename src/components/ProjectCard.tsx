 ...
 *   below. Used for the four middle projects.
 * - `featured`: a wider, two-column split at `sm+` (image/glyph left,
 *   copy right) that spans both grid columns — used to bookend the grid
 *   (first + last project) so six same-size tiles don't read as a flat
 *   Tailwind grid dump. Collapses back to the stacked layout below `sm`.
 *
 * `.above-grain` is applied directly to this component's root element
 * (the `<article>`): that class sets `position: relative; z-index: 10000;
 * isolation: isolate`, which is enough on its own to form a new stacking
 * context above the fixed `.grain-layer` (z-index 9999) — no transform or
 * motion wrapper is required for the fix to hold, so it survives being a
 * plain (non-framer) element here.
 */
    ...
    >
      <button
        ...
        className={`relative block text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] ${
          featured ? "sm:w-[44%] sm:shrink-0" : "w-full"
        }`}
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-7">
        <h3
          className={`font-display ${featured ? "text-[26px] sm:text-[32px]" : "text-[22px] sm:text-[24px]"}`}
